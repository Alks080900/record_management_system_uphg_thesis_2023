...
$result = mysqli_query($conn, "SELECT Firstname, Lastname FROM shsstud WHERE id=$id");
$row = mysqli_fetch_assoc($result);
$firstname = $row['Firstname'];
$lastname = $row['Lastname'];

// Update the user's status to 0
$sql = "UPDATE shsstud SET status=0, user_date='$user_date', am_pm='$am_pm' WHERE id=" . $id;
...
if(isset($_SESSION['login'])) {
    ...
    $action = "ARCHIVED $firstname $lastname IN SHS";
...
if ($conn->query($sql) === TRUE) {
    ...
    header("Location: searchstudent.php?status=archive_success");
    exit();
} else {
