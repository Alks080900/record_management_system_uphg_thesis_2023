...
$sql = "UPDATE collegestud SET Stud_ID=?, Lastname=?, Firstname=?, Middlename=?, Extensionname=?, Gender=?, Nationality=?, DoB=?, PoB=?, ContactNumber=?, Course=?, Email=?, Perm_Address=?, Year=?, user_date=DATE_FORMAT(?, '%Y-%m-%d %h:%i:%s %p'), am_pm=? WHERE id=?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("ssssssssssssssssi", $Stud_ID, $Lastname, $Firstname, $Middlename, $Extensionname, $Gender, $Nationality, $DoB, $PoB, $ContactNumber, $Course, $Email, $Perm_Address, $Year, $now_date, $now_ampm, $id);
$stmt->execute();
...
if (!empty($updatedFiles)) {
  ...
  $action = "UPDATED THE $updatedFilesStr OF $Firstname $Lastname IN COLLEGE";
} else {
  $action = "UPDATED $Firstname $Lastname IN COLLEGE";
}
...
if (isset($_SESSION['login'])) {
  ...
  if ($stmt->execute()) {
    ...
    echo 'swal("SUCCESS","'.$Lastname.' has been updated! Redirecting you...","success");';
...
if (isset($_FILES["PSA"]) && $_FILES["PSA"]["name"] != "") {
  ...
  if ($filetype == "application/pdf") {
    ...
    $sql = "SELECT * FROM collegestud WHERE id=?";
    $stmt = $conn->prepare($sql);
    ...
    $row = $result->fetch_assoc();
    if ($row["PSA"] != "") {
      unlink("C:/xampp/htdocs/Capstone/uploads/College/PSA/" . $row["PSA"]); // Delete the existing file
    }
    // Insert the new file
    $newfilename = $Stud_ID . ".pdf";
    move_uploaded_file($filetemp, "C:/xampp/htdocs/Capstone/uploads/College/PSA/" . $newfilename);
    $timestamp = date('Y-m-d H:i:s');
    $am_pm = date('A');
    $sql = "UPDATE collegestud SET PSA=?, user_date=DATE_FORMAT(?, '%Y-%m-%d %h:%i:%s %p'), am_pm=? WHERE id=?";
    $stmt = $conn->prepare($sql);
...
if (isset($_FILES["Form138"]) && $_FILES["Form138"]["name"] != "") {
  ...
  if ($filetype == "application/pdf") {
    ...
    $row = $result->fetch_assoc();
    if ($row["Form138"] != "") {
      unlink("C:/xampp/htdocs/Capstone/uploads/College/Form138/" . $row["Form138"]); // Delete the existing file
    }
    // Insert the new file
    $newfilename = $Stud_ID . ".pdf";
    move_uploaded_file($filetemp, "C:/xampp/htdocs/Capstone/uploads/College/Form138/" . $newfilename);
    $timestamp = date('Y-m-d H:i:s');
    $am_pm = date('A');
    $sql = "UPDATE collegestud SET Form138=?, user_date=DATE_FORMAT(?, '%Y-%m-%d %h:%i:%s %p'), am_pm=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssi", $newfilename, $timestamp, $am_pm, $ID);
  ...
  if (isset($_FILES["Form137"]) && $_FILES["Form137"]["name"] != "") {
    ...
    if ($filetype == "application/pdf") {
      ...
      $row = $result->fetch_assoc();
      if ($row["Form137"] != "") {
        unlink("C:/xampp/htdocs/Capstone/uploads/College/Form137/" . $row["Form137"]); // Delete the existing file
      }
      // Insert the new file
      $newfilename = $Stud_ID . ".pdf";
      move_uploaded_file($filetemp, "C:/xampp/htdocs/Capstone/uploads/College/Form137/" . $newfilename);
      $timestamp = date('Y-m-d H:i:s');
    $am_pm = date('A');
    $sql = "UPDATE collegestud SET Form137=?, user_date=DATE_FORMAT(?, '%Y-%m-%d %h:%i:%s %p'), am_pm=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssi", $newfilename, $timestamp, $am_pm, $ID);
      if ($stmt->execute()) {
        $ID = $_GET['id'];
  ...
  if (isset($_FILES["Picture"]) && $_FILES["Picture"]["name"] != "") {
    ...
    if ($filetype == "image/jpeg") {
      ...
      $row = $result->fetch_assoc();
      if ($row["Picture"] != "") {
        unlink("C:/xampp/htdocs/Capstone/uploads/College/Picture/" . $row["Picture"]); // Delete the existing file
      }
      // Insert the new file
      $newfilename = $Stud_ID . ".jpeg";
      move_uploaded_file($filetemp, "C:/xampp/htdocs/Capstone/uploads/College/Picture/" . $newfilename);
      $timestamp = date('Y-m-d H:i:s');
      $am_pm = date('A');
      $sql = "UPDATE collegestud SET Picture=?, user_date=DATE_FORMAT(?, '%Y-%m-%d %h:%i:%s %p'), am_pm=? WHERE id=?";
      $stmt = $conn->prepare($sql);
      $stmt->bind_param("sssi", $newfilename, $timestamp, $am_pm, $ID);
      if ($stmt->execute()) {
        $ID = $_GET['id'];
...
if (isset($_FILES["MOA"]) && $_FILES["MOA"]["name"] != "") {
  ...
  if ($filetype == "application/pdf") {
    ...
    $row = $result->fetch_assoc();
    if ($row["MOA"] != "") {
      unlink("C:/xampp/htdocs/Capstone/uploads/College/MOA/" . $row["MOA"]); // Delete the existing file
    }
    // Insert the new file
    $newfilename = $Stud_ID . ".pdf";
    move_uploaded_file($filetemp, "C:/xampp/htdocs/Capstone/uploads/College/MOA/" . $newfilename);
    $timestamp = date('Y-m-d H:i:s');
    $am_pm = date('A');
    $sql = "UPDATE collegestud SET MOA=?, user_date=DATE_FORMAT(?, '%Y-%m-%d %h:%i:%s %p'), am_pm=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssi", $newfilename, $timestamp, $am_pm, $ID);
...
<div id="img-container"></div>
	<h2>College Update Student Form <button type="button" class="btn btn-primary btn-md" data-bs-toggle="modal" data-bs-target="#staticBackdrop" style="position: absolute; margin-left: 10px;" id="EDIT"><i class="bi bi-pencil-fill" style="margin-right: 10px;"></i>Edit</button></h2>
</button>
    <br>
    ...
    <div class="form-group col-lg-3">
        ...
        <div class="form-group col-lg-3">
            <label for="MiddleIni">Middle Name</label>
          ...
          <div class="form-group col-lg-3">
            <label for="extentionname">Extension Name</label>
          ...
          <div class="form-group col-lg-3">
            ...
            <select class="form-control form-select" class="course1" name="Course" id="course123" style="margin-top:15px;">
  ...
  <div class="row">
    <div class="col-md-4">
      <div class="custom-file">
        <?php if ($PSA): ?>
          <label>PSA/NSO</label>
          <a href="/Capstone/uploads/College/PSA/<?php echo $PSA; ?>" target="_blank"><?php echo $PSA; ?></a>
        <?php else: ?>
    ...
    <div class="col-md-4">
      <div class="custom-file">
        <?php if ($Form138): ?>
          <label>Form138</label>
          <a href="/Capstone/uploads/College/Form138/<?php echo $Form138; ?>" target="_blank"><?php echo $Form138; ?></a>
        <?php else: ?>
    ...
    <div class="col-md-4">
      <div class="custom-file">
        <?php if ($Form137): ?>
          ...
          <a href="/Capstone/uploads/College/Form137/<?php echo $Form137; ?>" target="_blank"><?php echo $Form137; ?></a>
        <?php else: ?>
    ...
    <div class="col-md-4">
      <div class="custom-file">
        <?php if ($Picture): ?>
          ...
          <a href="/Capstone/uploads/College/Picture/<?php echo $Picture; ?>" target="_blank"><?php echo $Picture; ?></a>
        <?php else: ?>
    ...
    <div class="col-md-4">
      <div class="custom-file">
        <?php if ($MOA): ?>
          <label>MOA</label>
          <a href="/Capstone/uploads/College/MOA/<?php echo $MOA; ?>" target="_blank"><?php echo $MOA; ?></a>
        <?php else: ?>
...
<script>
    ...
    img.src = "/Capstone/uploads/College/Picture/<?php echo $Picture; ?>";
